...
(function(window,undefined){
	 ...
	 * @return jsonDB
	 */
	var jsonDB = function(dbName,data){

		//创建数据库或者数据表
		if(data){
			dbName = dbName || 'json_db';
			eval('Database.'+ dbName +'= data');
			window.localStorage.Database = JSON.stringify(Database);
		}else if(window.localStorage.Database){
			Database = JSON.parse(window.localStorage.Database);
		}
